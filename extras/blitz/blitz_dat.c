...
static const char *titanStrings[] = {
#include "blitz_titan_files.h"
};
static const unsigned int numTitanStrings = PL_ARRAY_ELEMENTS( titanStrings );

static const char *GetStringForHash( int32_t hash, const char **strings, unsigned int numStrings ) {
	if ( strings == NULL ) {
		return NULL;
	}

	for ( unsigned int i = 0; i < numStrings; ++i ) {
		unsigned int sHash = HashString( strings[ i ] );
		if ( sHash != hash ) {
			continue;
		}
...
static PLPackage *ParseDATFile( PLFile *file ) {
	...
	if ( filePath != NULL ) {
		...
		if ( fileName != NULL ) {
			if ( pl_strcasecmp( fileName, "TITAN.DAT" ) == 0 ) {
	...
	for ( unsigned int i = 0; i < numFiles; ++i ) {
		...
		const char *fileName = GetStringForHash( hash, strings, numStrings );
		if ( fileName == NULL ) {
	...
	printf( "%.2lf%% matched (%u/%u) for Blitz package \"%s\"\n", ( ( double ) numMatches / numFiles ) * 100.0,
	        numMatches, numFiles,
	        PlGetFileName( PlGetFilePath( file ) ) );
#endif
