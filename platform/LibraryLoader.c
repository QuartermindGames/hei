...
PLLibrary *plLoadLibrary( const char *path, bool appendPath ) {
	...
	if ( appendPath ) {
		snprintf( "%s.%s", sizeof( sysPath ), path, PL_SYSTEM_LIBRARY_EXTENSION );
	} else {
	...
	if ( libraryHandle == NULL ) {
		ReportError( PL_RESULT_INVALID_PARM1, "failed to load library (%d)", GetLastError() );
	}
#else /* unix */
...
void *plGetLibraryProcedure( PLLibrary *library, const char *procedureName ) {
	...
	if ( myProcedure == NULL ) {
		ReportError( PL_RESULT_INVALID_PARM2, "failed to find procedure (%d)", GetLastError() );
	}
#else /* unix */
...
void plUnloadLibrary( PLLibrary *library ) {
	...
	if ( !FreeLibrary( ( HMODULE ) library ) ) {
		ReportError( PL_RESULT_INVALID_PARM1, "failed to unload library (%d)", GetLastError() );
	}
#else /* unix */
