...
PLModel* LoadAnimatedRequiemModel(PLFile* fp) {
  plRewindFile(fp);
...
PLModel* LoadStaticRequiemModel(PLFile* fp) {
  plRewindFile(fp);
  ...
  if (plReadFile(fp, &texture_name_length, sizeof(uint32_t), 1) != 1) {
    ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to get texture name length");
    return NULL;
  }

  if (texture_name_length > MAX_TEXTURE_NAME || texture_name_length == 0) {
    ReportError(PL_RESULT_FILEREAD, "invalid texture name length, %d", texture_name_length);
    return NULL;
  }
  ...
  if (plReadFile(fp, &num_vertices, sizeof(uint32_t), 1) != 1) {
    ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to get number of vertices");
    return NULL;
  }
  ...
  if (plReadFile(fp, &num_polygons, sizeof(uint32_t), 1) != 1) {
    ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to get number of polygons");
    return NULL;
  }
  ...
  for (unsigned int i = 0; i < num_polygons; ++i) {
    ...
    if (plReadFile(fp, &polygons[i].num_indices, sizeof(uint32_t), 1) != 1) {
      ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to load number of indices! (offset: %ld)", plGetFileOffset(fp));
      return NULL;
    }

    if (polygons[i].num_indices < MIN_INDICES_PER_POLYGON || polygons[i].num_indices > MAX_INDICES_PER_POLYGON) {
      ReportError(PL_RESULT_FILEREAD, "invalid number of indices, %d, required for polygon %d! (offset: %ld)",
    ...
    if (plReadFile(fp, polygons[i].indices, sizeof(uint16_t), polygons[i].num_indices) != polygons[i].num_indices) {
      ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to load indices");
      return NULL;
    }
    ...
    if (plReadFile(fp, polygons[i].uv, sizeof(int16_t), num_uv_coords) != num_uv_coords) {
      ReportError(PL_RESULT_FILEREAD, "invalid file length, failed to load UV coords");
      return NULL;
    }
