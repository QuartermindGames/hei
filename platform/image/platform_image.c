...
void plClearImageLoaders( void ) {
	memset( imageLoaders, 0, sizeof( PLImageLoader ) * MAX_IMAGE_LOADERS );
	numImageLoaders = 0;
}
...
PLImage *plLoadImage( const char *path ) {
	...
	ReportBasicError( PL_RESULT_UNSUPPORTED );
...
bool plWriteImage( const PLImage *image, const char *path ) {
	...
	int comp = ( int ) plGetSamplesPerPixel( image->colour_format );
	if ( comp == 0 ) {
